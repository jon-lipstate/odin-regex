...
NFA create_nfa()
{
    ...
    Transition t0[] = {{.to = 1, .match = {.type = MATCH_RANGE, .min = '0', .max = '9', .invert = 0}}};
    nfa.transitions[0] = (Transition *)malloc(sizeof(t0));
    memcpy(nfa.transitions[0], t0, sizeof(t0));

    Transition t1[] = {{.to = 0, .match = {.type = MATCH_EPSILON}},
                       {.to = 2, .match = {.type = MATCH_RANGE, .min = '-', .max = '-', .invert = 0}}};
    nfa.transitions[1] = (Transition *)malloc(sizeof(t1));
    memcpy(nfa.transitions[1], t1, sizeof(t1));

    Transition t2[] = {{.to = 3, .match = {.type = MATCH_RANGE, .min = '0', .max = '9', .invert = 0}}};
    nfa.transitions[2] = (Transition *)malloc(sizeof(t2));
    memcpy(nfa.transitions[2], t2, sizeof(t2));

    Transition t3[] = {{.to = 2, .match = {.type = MATCH_EPSILON}},
                       {.to = 4, .match = {.type = MATCH_RANGE, .min = '-', .max = '-', .invert = 0}}};
    nfa.transitions[3] = (Transition *)malloc(sizeof(t3));
    memcpy(nfa.transitions[3], t3, sizeof(t3));

    Transition t4[] = {{.to = 5, .match = {.type = MATCH_RANGE, .min = '0', .max = '9', .invert = 0}}};
...
int main()
{
    // TODO: This code was whipped up and is probably buggy, its not matching correct for sure
    // Verify NFA more closely, double check all procs.
